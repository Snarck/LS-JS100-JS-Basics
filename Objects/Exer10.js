...
Note that all 3 properties have the same value. However,
what happens if we modify these values in one of the two objects and then inspect both objects?
...
but not in obj. However, when we mutated the value of objCopy.array, it also mutated that value of obj.array.
...
If you're wondering why this happens, you might want to review Variables as Pointers section in the Introduction to JavaScript book.
...
function clone(obj) {
 // All variables point to the same place.  return Object.create(obj);
 let cloner = {};
 return Object.assign (cloner, obj);
}
...
console.log(person.name.isStageName);       // false
console.log(clonedPerson.name.isStageName); // false
//I could have created the object in the params in line 66. 
